...
func makeArrayConsecutive2(statues []int) (nMissing int) {
	sort.Ints(statues)
	for i, length := 0, len(statues); i < length-1; i++ {
		statue, adjacent := statues[i], statues[i+1]
		nMissing += adjacent - statue - 1
	}
	return nMissing
}
