...
Input/Output
    ...
    [output] array.array.integer
		A blurred image represented as integers, obtained through the process
		in the description.


*/

func boxBlur(image [][]int) (blurred [][]int) {
	nColumn, nRows := len(image), len(image[0])
	for y := 1; y < nColumn-1; y++ {
		col1, col2, col3 := image[y-1], image[y], image[y+1]
		blurs := make([]int, nRows-2) // alloting nRows less first & last.
		for x := 1; x < nRows-1; x++ {
			row1, row2, row3 := col1[x-1:x+2], col2[x-1:x+2], col3[x-1:x+2]
			blurs[x-1] = (sum(row1) + sum(row2) + sum(row3)) / 9
		}
		blurred = append(blurred, blurs)
	}
	return
}
